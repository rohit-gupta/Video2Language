...
encoded_captions = pickle.load(open("encoded_captions_unverified_len_4_10.p", "rb"))
...
lang_model.compile(optimizer=sgd,
              loss='categorical_crossentropy',
              metrics=['accuracy'])

lang_model.load_weights("../models/language_model_3layers_len_4_10_best_weights_dropout_0p5_finetuned7.h5")

lang_model.summary()

csv_logger = CSVLogger('../logs/language_model_3layers_len_4_10_training_dropout_0p5_finetuned8.log')
checkpointer = ModelCheckpoint(filepath='../models/language_model_3layers_len_4_10_best_weights_dropout_0p5_finetuned8.h5', verbose=1, save_best_only=True)

lang_model.fit([X1_train, X2_train, X3_train, X4_train], Y_train, epochs=200, batch_size=100, validation_split=0.1, callbacks=[csv_logger, checkpointer])
lang_model.save('../models/language_model_3layers_len_4_10_dropout_0p5_finetuned8.h5')
...
hot_sentence = []
for word in preds[0]:
	hot_sentence.append(vocabulary[sample(word, 0.2)][1])
